      ...
      try {         
        ...
        if (!urls) return client.reply(m.chat, Func.texted('bold', `🚩 Mungkin pesan yang Anda balas tidak berisi hasil penelusuran dari youtube.`), m)
         ...
         if (!json.status) return client.reply(m.chat, global.status.fail, m)
            ...
            if (chSize.oversize) return client.reply(m.chat, `💀 Ukuran file (${json.data.size}) melebihi batas maksimum, unduh sendiri melalui tautan ini : ${await (await Scraper.shorten(json.data.url)).data.url}`, m)
